...
CANFrame = namedtuple('CANFrame',
        ['sentinel_start', 'sequence', 'id', 'rtr', 'length', 'data', 'sentinel_end'])
...
class ArduinoSource(object):
    ...
    def _unpack_packet(self, data):
        (sentinel_start, sequence, can_id, rtr, length, sentinel_end) = \
                unpack(self.FORMAT, data)
        can_data = np.fromstring(data[7:15], dtype=np.uint8)[:length]
        return CANFrame(sentinel_start, sequence, can_id, rtr, length, can_data, sentinel_end)
        ...
        while True:
            ...
            if self._is_sync_packet(data):
                ...
                if not self._is_valid_frame(frame):
                    ...
                    self._synchronize("sentinel 0x%02X 0x%02X" % (frame.sentinel_start, frame.sentinel_end) if 0 <= frame.length <= 8 else "length")
                    continue
                yield frame
